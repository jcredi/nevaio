...
- Object selection (spec section 7) reads Nevaio's own static OSM object index, never MapTiler's rendered feature properties. `docs/research/maptiler-outdoor-objects.md` is the measured reason and is worth reading before touching any of it: the Outdoor style renders no saddles at all, only `rank == 1` peaks (about a quarter of them, and `rank` changes with zoom), and symbol collision culls most of the rest. `app/src/objects/objectIndexSchema.ts` validates the sharded index the way `manifestSchema.ts` validates `latest.json` - shard paths must stay on the index URL's own origin and directory - and `selection.ts` holds the tap-to-record rule and its failure modes. Shards load lazily by viewport; the fixture in `app/public/object-index/` is a real cut of the published artifact and goes away when the R2 URL is wired up.
- The historical AS-OF date picker (spec section 5.3) reads `dates.json`, published beside `latest.json`; its URL is *derived* from `VITE_SNOW_MANIFEST_URL` by `dateCatalogueUrlFor`, so that one variable stays the single trust anchor for the snow layer - don't add a second one. `app/src/map/dateCatalogueSchema.ts` validates it as a public contract and mirrors `validate_date_catalogue` in the pipeline; when one side's rules change, both move. An archived date manifest is `latest.json`-shaped and lives in the same directory, so `validateTileManifest` accepts it unchanged - keep it that way. Fewer than two available dates renders a plain label rather than a one-option select. A local preview wants `app/public/snow/dates.json` and `asof-<date>-<runId>.json` beside `latest.json`; all three are gitignored.
- The three network-facing validators (`map/manifestSchema.ts`, `map/dateCatalogueSchema.ts`, `objects/objectIndexSchema.ts`) each keep their own copies of the URL-trust and primitive-check helpers. That duplication is deliberate: each one reads and audits end to end on its own. Don't consolidate them into a shared module.
...
- Entry points are `python -m nevaio_pipeline.render` (was `pipeline.preview`), `python -m nevaio_pipeline.publish` and `python -m nevaio_pipeline.publish_object_index`. `pipeline/src/nevaio_pipeline/__init__.py` exports lazily on purpose - the publisher must import without the native raster stack, so never add an eager `from .tiles import ...` there.
