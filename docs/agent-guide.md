...
- Snow overlay assets live in `app/public/snow/` and are generated by `pipeline/tools/make_sample_overlay.py` - don't hand-edit them. The `.json` sidecar carries the corner coordinates, so swapping products means changing one URL in `config.ts`.
...
- Findings go in `docs/research/gfsc-findings.md` as you go - short bullet notes, not a formal write-up.
- `pipeline/tools/make_sample_overlay.py` turns one GFSC `GF.tif` into the committed fallback overlay in `app/public/snow/`, and self-verifies its georeferencing on every run (11 landmarks + 300 random points). It's the one script that writes into `app/`. Takes `--input` / `--output-dir`; the defaults reproduce the committed asset byte-for-byte from `data/research/`.
- That sample is deliberately GF-only, so it has no observation-age channel, unlike production. Don't "fix" that difference - see `docs/spec.md` section 5.2.
